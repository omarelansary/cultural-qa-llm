...
Usage example:
  python finetune_mcq_saq_lora.py \
    --mcq_csv /mnt/data/train_dataset_mcq.csv \
    --saq_csv /mnt/data/train_dataset_saq.csv \
...
def build_mcq_prompt(country: str, raw_prompt: str) -> str:
    ...
    return (
        "### Instruction:\n"
        "Answer the multiple-choice question. Reply with the letter only (A, B, C, or D).\n\n"
        f"### Context:\nCountry: {country}\n\n"
        "### Question:\n"
        f"{raw_prompt}\n\n"
        f"{RESPONSE_MARKER}"
...
def main():
    ...
    for _, r in mcq_df.iterrows():
        ...
        answer = str(r.get("answer_idx", "")).strip().upper()  # A/B/C/D
        ...
        rows.append(
            {
                ...
                "full_text": full_text,
                "gold_letter": answer,
                "gold_text": answer,  # for uniformity
            }
        )
    ...
    for _, r in saq_df.iterrows():
        ...
        full_text = prompt_only + canonical
